...
OUTPUT_BASE = Path("/workspace/data/output/set50Hibrid/ablation_h_f1_t3_nn7")
...
def selected_levels():
    raw = os.environ.get("SET50_HIBRID_ABLATION_LEVELS")
    if not raw:
        return LEVELS
...
def h_multipliers():
    raw = os.environ.get("SET50_HIBRID_ABLATION_H_MULTIPLIERS")
    if not raw:
...
def max_images():
    raw = os.environ.get("SET50_HIBRID_ABLATION_MAX_IMAGES")
    if not raw:
        return None
    return int(raw)
...
def images_per_level():
    raw = os.environ.get("SET50_HIBRID_ABLATION_IMAGES_PER_LEVEL")
    if raw is None:
        return 2
    raw = raw.strip().lower()
    if raw in {"", "all", "none"}:
...
def sample_seed():
    return int(os.environ.get("SET50_HIBRID_ABLATION_SAMPLE_SEED", "42"))


def force_run():
    return os.environ.get("SET50_HIBRID_ABLATION_FORCE", "0") == "1"


def n_jobs():
    raw = os.environ.get("SET50_HIBRID_ABLATION_N_JOBS")
    if not raw:
        return -1
    return int(raw)
...
def read_or_run_hybrid(item, out_path, h_multiplier):
...
def run_level(level, baselines):
    ...
    for h_multiplier in h_multipliers():
        ...
        save_pickle(level_variant_records, results_dir, f"results_set50_hibrid_ablation_{level}_{tag}.pkl")
        save_results_to_xlsx(
            level_variant_records,
            results_dir,
            f"results_set50_hibrid_ablation_{level}_{tag}.xlsx",
        )
...
def make_summary(records, output_dir):
    ...
    if df.empty:
        ...
        save_results_to_xlsx(summary.to_dict("records"), output_dir, "results_set50_hibrid_h_ablation_summary.xlsx")
        save_results_to_xlsx(best.to_dict("records"), output_dir, "results_set50_hibrid_h_ablation_best_by_level.xlsx")
        save_results_to_xlsx(overall.to_dict("records"), output_dir, "results_set50_hibrid_h_ablation_overall.xlsx")
        save_pickle(summary.to_dict("records"), output_dir, "results_set50_hibrid_h_ablation_summary.pkl")
        save_pickle(best.to_dict("records"), output_dir, "results_set50_hibrid_h_ablation_best_by_level.pkl")
        save_pickle(overall.to_dict("records"), output_dir, "results_set50_hibrid_h_ablation_overall.pkl")
        return summary, best
    ...
    best = (
        summary.sort_values(["level", "mean_score_geohibnlm"], ascending=[True, False])
        .groupby("level", as_index=False)
        .head(1)
        .reset_index(drop=True)
    )
    overall = (
        df.groupby(["h_multiplier", "ablation_name", "f", "t", "nn"])
        .agg(
            mean_psnr_geohibnlm=("psnr_geohibnlm", "mean"),
            std_psnr_geohibnlm=("psnr_geohibnlm", "std"),
            sem_psnr_geohibnlm=("psnr_geohibnlm", "sem"),
    ...
    save_results_to_xlsx(
        ...
        output_dir,
        "results_set50_hibrid_h_ablation_selected_images.xlsx",
    )
    return summary, best
...
if __name__ == "__main__":
    ...
    save_pickle(all_records, summary_dir, "results_set50_hibrid_h_ablation_all.pkl")
    save_results_to_xlsx(
        all_records,
        summary_dir,
        "results_set50_hibrid_h_ablation_all.xlsx",
    )
