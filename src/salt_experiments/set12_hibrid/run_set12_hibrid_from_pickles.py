...
def create_method_format_xlsx(all_records, out_path):
    ...
    for level in LEVELS:
        ...
        if not rows:
            continue
        ws = wb.create_sheet(f"Hibrid Set12 {level.title()}")
        ws.append(header)
        for record in rows:
            ws.append([
                None,
...
def create_pdfs(summary, out_old, out_new):
    ...
    with PdfPages(out_old) as pdf:
        add_table_page(pdf, "Set12 Hibrid - Summary Score", summary, old_cols)
    ...
    with PdfPages(out_new) as pdf:
        add_table_page(pdf, "Set12 Hibrid - Score Format", summary, new_cols_score)
        add_table_page(pdf, "Set12 Hibrid - PSNR Format", summary, new_cols_psnr)
        add_table_page(pdf, "Set12 Hibrid - SSIM Format", summary, new_cols_ssim)
...
if __name__ == "__main__":
    ...
    method_xlsx = create_method_format_xlsx(
        all_records,
        summary_dir / "Results Salt and Pepper Hibrid Method - Set12.xlsx",
    )
    create_pdfs(
        summary,
        summary_dir / "results_set12_hibrid_summary_old_format.pdf",
        summary_dir / "results_set12_hibrid_method_format.pdf",
    )
