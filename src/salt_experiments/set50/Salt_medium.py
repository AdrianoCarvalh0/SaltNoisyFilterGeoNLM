...
def generate_salt_experiment_medium(parameters):
    """
    Run the medium-noise salt-and-pepper denoising experiment using NLM, GEO-NLM,
    median filtering, and ASWMF.
    
    The function:
      1. Reads all images in dir_images_general.
      2. Adds medium-level salt-and-pepper noise.
    ...
    root_dir_output_medium = parameters['root_dir_output_medium']
    ...
    name_pickle_nlm_output_medium = parameters['name_pickle_nlm_output_medium']
    name_pickle_results_gnlm_median_aswmf_output_medium = parameters['name_pickle_results_gnlm_median_aswmf_output_medium']
    name_results_xlsx_nlm_gnlm_median_aswmf_output_medium = parameters['name_results_xlsx_nlm_gnlm_median_aswmf_output_medium']
    ...
    array_nlm_medium_filtereds = []

    # ---------------------------------------
    # 1) NLM PHASE — medium SALT-AND-PEPPER NOISE
    # ---------------------------------------
    for file in array_dir:
        ...
        # Add medium salt-and-pepper noise to create the noisy observation
        ...
        q_nlm_candidates = np.array([h_nlm + delta for delta in range(125, 225, 1)])
        ...
        array_nlm_medium_filtereds.append(dct)
    ...
    save_pickle(array_nlm_medium_filtereds, dir_out_results, name_pickle_nlm_output_medium)

    # --------------------------------------------------
    # 2) MEDIAN, ASWMF, AND GEO-NLM PHASE — medium SALT-AND-PEPPER NOISE
    # --------------------------------------------------

    array_gnlm_median_aswmf_medium_filtereds = []

    # Reload the NLM results to use them as input for GEO-NLM, median, and ASWMF
    vector = load_pickle(dir_out_results, name_pickle_nlm_output_medium)
    ...
    for array in vector:
        ...
        array_gnlm_median_aswmf_medium_filtereds.append(dict_results)
