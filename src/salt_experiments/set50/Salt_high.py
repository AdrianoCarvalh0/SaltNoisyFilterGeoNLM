...
def generate_salt_experiment_high(parameters):
    """
    Run the high-noise salt-and-pepper denoising experiment using NLM, GEO-NLM,
    median filtering, and ASWMF.
    
    The function:
      1. Reads all images in dir_images_general.
      2. Adds high-level salt-and-pepper noise.
    ...
    name_pickle_results_gnlm_median_aswmf_output_high = parameters['name_pickle_results_gnlm_median_aswmf_output_high']
    name_results_xlsx_nlm_gnlm_median_aswmf_output_high = parameters['name_results_xlsx_nlm_gnlm_median_aswmf_output_high']
    ...
    array_nlm_high_filtereds = []

    # ---------------------------------------
    # 1) NLM PHASE — high SALT-AND-PEPPER NOISE
    # ---------------------------------------
    for file in array_dir:
        ...
        # Add high salt-and-pepper noise to create the noisy observation
        ...
        q_nlm_candidates = np.array([h_nlm + delta for delta in range(225, 325, 1)])
        ...
        array_nlm_high_filtereds.append(dct)
    ...
    save_pickle(array_nlm_high_filtereds, dir_out_results, name_pickle_nlm_output_high)

    # --------------------------------------------------
    # 2) MEDIAN, ASWMF, AND GEO-NLM PHASE — high SALT-AND-PEPPER NOISE
    # --------------------------------------------------

    array_gnlm_median_aswmf_high_filtereds = []
    ...
    for array in vector:
        ...
        array_gnlm_median_aswmf_high_filtereds.append(dict_results)
