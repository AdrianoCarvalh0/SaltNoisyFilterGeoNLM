...
def generate_salt_experiment_extreme(parameters):
    """
    Run the extreme-noise salt-and-pepper denoising experiment using NLM, GEO-NLM,
    median filtering, and ASWMF.
    
    The function:
      1. Reads all images in dir_images_general.
      2. Adds extreme-level salt-and-pepper noise.
    ...
    root_dir_output_extreme = parameters['root_dir_output_extreme']
    dir_images_general = parameters['dir_images_set50']
    ...
    name_pickle_nlm_output_extreme = parameters['name_pickle_nlm_output_extreme']
    name_pickle_results_gnlm_median_aswmf_output_extreme = parameters['name_pickle_results_gnlm_median_aswmf_output_extreme']
    name_results_xlsx_nlm_gnlm_median_aswmf_output_extreme = parameters['name_results_xlsx_nlm_gnlm_median_aswmf_output_extreme']
    ...
    array_nlm_extreme_filtereds = []

    # ---------------------------------------
    # 1) NLM PHASE — extreme SALT-AND-PEPPER NOISE
    # ---------------------------------------
    for file in array_dir:
        ...
        # Add extreme salt-and-pepper noise to create the noisy observation
        ...
        q_nlm_candidates = np.array([h_nlm + delta for delta in range(400, 500, 1)])
        ...
        array_nlm_extreme_filtereds.append(dct)
    ...
    save_pickle(array_nlm_extreme_filtereds, dir_out_results, name_pickle_nlm_output_extreme)

    # --------------------------------------------------
    # 2) MEDIAN, ASWMF, AND GEO-NLM PHASE — extreme SALT-AND-PEPPER NOISE
    # --------------------------------------------------

    array_gnlm_median_aswmf_extreme_filtereds = []

    # Reload the NLM results to use them as input for GEO-NLM, median, and ASWMF
    vector = load_pickle(dir_out_results, name_pickle_nlm_output_extreme)
    ...
    for array in vector:
        ...
        array_gnlm_median_aswmf_extreme_filtereds.append(dict_results)

    # Save the combined GEO-NLM, median, and ASWMF results to pickle
    save_pickle(
        array_gnlm_median_aswmf_extreme_filtereds,
        dir_out_results,
        name_pickle_results_gnlm_median_aswmf_output_extreme
    )
    ...
    save_results_to_xlsx(
        array_gnlm_median_aswmf_extreme_filtereds,
        dir_out_results,
        name_results_xlsx_nlm_gnlm_median_aswmf_output_extreme
    )
